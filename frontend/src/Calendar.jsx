...
const Calendar = () => {
  ...
  const [latestLowestWeightDate, setLatestLowestWeightDate] = useState(null);
  ...
  useEffect(() => {
    if (logs.length > 0) {
      ...
      // Determine latest date with lowest weight
      const latestWeightEntry = logs
        .filter(l => l.weight === lowestWeight)
        .sort((a, b) => new Date(b.date) - new Date(a.date))[0];
      setLatestLowestWeightDate(latestWeightEntry?.date || null);
    }
  }, [logs]);
  ...
  const renderDayCell = (date) => {
    ...
    const isLatestLowestWeight = entryForDay && entryForDay.date === latestLowestWeightDate;
    ...
    return (
      ...
      >
        {isStepsPB && (
          <md-icon className="absolute top-1 left-1 text-yellow-400 text-3xl">emoji_events</md-icon>
        )}
        {isLatestLowestWeight && (
          <md-icon className="absolute top-1 right-1 text-red-500 text-3xl">local_fire_department</md-icon>
        )}
  ...
  const renderYearViewDayCell = (date, isCurrentMonth) => {
    ...
    return (
      <div key={dateStr} onClick={() => isCurrentMonth && !isFutureDate && handleDateClick(dateStr)} className={className}>
        {isStepsPB && (
          <md-icon className="absolute top-0.5 left-0.5 text-yellow-400 text-3xl">emoji_events</md-icon>
        )}
        {isLatestLowestWeight && (
          <md-icon className="absolute top-0.5 right-0.5 text-red-500 text-3xl">local_fire_department</md-icon>
        )}
